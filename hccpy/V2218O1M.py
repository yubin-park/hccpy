...
def create_interactions(cc_lst, DISABL):
    ...
    z["RENAL"] = max(x["HCC134"], x["HCC135"], x["HCC136"], x["HCC137"])
    z["SEPSIS"] = x["HCC2"]
    z["gSubstanceAbuse"] = max(x["HCC54"], x["HCC55"])
    z["gPsychiatric"] =  max(x["HCC57"], x["HCC58"])
